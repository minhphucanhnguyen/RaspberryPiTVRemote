...
class NEC():
    def __init__(self,
                ...
                leading_gap_duration=4440,
                one_pulse_duration = 597,
                one_gap_duration = 1655,
                zero_pulse_duration = 597,
                zero_gap_duration = 524,
                trailing_pulse = 0):
        self.master = master
...
if __name__ == "__main__":
    import pyrebase
	# Setting Firebase API
    config = {
                "apiKey":"AIzaSyCMyxGyFPbJ5ZlL3XNmFHEmzh4DuZMZ1m",
                "authDomain":"btl1-50022.firebaseapp.com",
                "databaseURL":"https://btl1-50022.firebaseio.com",
                "storageBucket":"btl1-50022.appspot.com",
    }
    ...
    ButtonID = ""
    index = -1
    gpio_pin = 23 # Setting output pin = GPIO23
    protocol = "NEC"
    firebase = pyrebase.initialize_app(config)
    db = firebase.database()
    while(1):
        data = db.child("message").get() # Get data from firebase
        recData = str(data.val()) # Convert received data to string
        recData = recData.split("|") # Split the string base on "|" indicator
		
        if (recData[0] != TV or recData[1] != RemoteID or eval(recData[2]) != ButtonID): # Check if choosing another remote model or button
            ...
            TV = recData[0]
            RemoteID = recData[1]
            ButtonID = eval(recData[2])
            print recData[3]
            index = 1
    
			# Open the file base on TV and RemoteID
            file = open("IR-code/" + TV + "/" + RemoteID + ".txt").read().splitlines()
            irCode = file[ButtonID + 6]
            print("Reading code: " + irCode.split("|")[1])
    
            protocol_config = dict(leading_pulse_duration = int(file[HEADER_MARK].split("|")[1]),
				        leading_gap_duration = int(file[HEADER_SPACE].split("|")[1]),
						one_pulse_duration = int(file[BIT_MARK].split("|")[1]),
						one_gap_duration = int(file[ONE_SPACE].split("|")[1]),
						zero_pulse_duration = int(file[BIT_MARK].split("|")[1]),
						zero_gap_duration = int(file[ZERO_SPACE].split("|")[1]),
						trailing_pulse = int(file[TRAILING_PULSE].split("|")[1]))

            ir = IR(gpio_pin, protocol, protocol_config)
            ir.send_code(bin(int(irCode.split("|")[1], 16))[2:].zfill(32))
            print("Exiting IR")
            index += 1

        elif (TV == recData[0] and RemoteID == recData[1] and ButtonID == eval(recData[2])): # Check if the same remote with the button is pressed again
            if (int(recData[3]) > index):

                loopNo = int(recData[3]) - index
                while (loopNo > 0):
                    file = open("IR-code/" + TV + "/" + RemoteID + ".txt").read().splitlines()
                    irCode = file[ButtonID + 6]
                    print("Reading code: " + irCode.split("|")[1])
                    ...
                    index += 1
                    loopNo -= 1
            elif (int(recData[3]) < index):
                loopNo = index - int(recData[3])
                while (loopNo > 0):
                    file = open("IR-code/" + TV + "/" + RemoteID + ".txt").read().splitlines()
                    irCode = file[ButtonID + 6]
                    print("Reading code: " + irCode.split("|")[1])
